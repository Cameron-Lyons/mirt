...
#[pyfunction]
fn lord_wingersky_recursion<'py>(
    py: Python<'py>,
    ...
    // Initialize log distribution: log P(sum = s | theta)
    // Shape: (max_score + 1, n_quad)
    let mut log_dist = Array2::from_elem((max_score + 1, n_quad), f64::NEG_INFINITY);

    // Base case: P(sum = 0 | theta, no items) = 1
    for q in 0..n_quad {
        log_dist[[0, q]] = 0.0;
    }
    ...
    for j in 0..n_items {
        ...
        // Compute log P(X_j = 1 | theta) and log P(X_j = 0 | theta) for all theta
        ...
        // Create new distribution by convolving with item response
        let mut new_log_dist = Array2::from_elem((max_score + 1, n_quad), f64::NEG_INFINITY);

        for s in 0..=max_score {
            for q in 0..n_quad {
                // P(sum = s) = P(sum = s, X_j = 0) + P(sum = s, X_j = 1)
                //            = P(sum = s | no j) * P(X_j = 0) + P(sum = s-1 | no j) * P(X_j = 1)
...
#[pyfunction]
fn lord_wingersky_polytomous<'py>(
    py: Python<'py>,
    ...
    for j in 0..n_items {
        ...
        for s in 0..=max_score {
            for q in 0..n_quad {
                // Sum over all category contributions
...
#[pyfunction]
#[allow(clippy::too_many_arguments)]
fn eapsum_from_distribution<'py>(
    py: Python<'py>,
    ...
    for i in 0..n_persons {
        ...
        // Compute log posterior: log P(theta | score) ∝ log P(score | theta) + log P(theta)
        let mut log_posterior = Vec::with_capacity(n_quad);
        for q in 0..n_quad {
            log_posterior.push(log_p[[s, q]] + log_prior[q]);
        }
        ...
        for q in 0..n_quad {
            theta_eap += posterior[q] * theta_points[q];
        }
        ...
        for q in 0..n_quad {
            let diff = theta_points[q] - theta_eap;
            variance += posterior[q] * diff * diff;
        }
        theta_se[i] = variance.sqrt();
    }
